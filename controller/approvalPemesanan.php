...
if (isset($_GET['idPemesanan'])) {
    ...
    if ($status == 'rejected') {
        ...
        if ($rejected) {
            header('Location: ../pages/admin-pemesanan.php?approval=rejected');
        } else {
            header('Location: ../pages/admin-pemesanan.php?approval=gagal');
        }
    } else {
        $approved = $koneksi->query("UPDATE pemesanan SET status_pemesanan='approved' WHERE id_pemesanan='" . $idPemesanan . "'");

        if ($approved) {
            header('Location: ../pages/admin-pemesanan.php?approval=approved');
        } else {
            header('Location: ../pages/admin-pemesanan.php?approval=gagal');
        }
    }
