document.querySelectorAll(".datepicker-container").forEach((container) => {
  ...
  applyBtn.addEventListener("click", () => {
    ...
    container.querySelectorAll("#paketList div").forEach(p => {
      p.classList.add(
        "bg-[#0a2008]",
        "hover:bg-[#237219]",
    ...
    fetch(`../controller/cek_slot.php?tanggal=${tanggal}`)
      ...
      .then((data) => {
        ...
        data.forEach(slot => {
          container.querySelectorAll("#paketList div").forEach(p => {
            console.log("p.dataset.idPaket: ", p.dataset.idPaket, "\nslot.id_paket: ",slot.id_paket);
            if (p.dataset.idPaket == slot.id_paket) {
              ...
              p.classList.add("bg-neutral-700", "text-neutral-800");
  ...
  const createButton = (text, isDisabled = false) => {
    ...
    if (!isDisabled) {
      ...
      fetch(
        `../controller/cek_slot_tanggal.php?tanggal=${now}&lapangan=${window.id_lapangan}`
      )
        .then((response) => response.json())
        .then((data) => {
          console.log(data);
